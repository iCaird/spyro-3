...
Spots where checksums would go are also located in the cutscene overlays, but these appear to be totally uncorrected (i.e. these are unmodified jump instructions in the final game, and no anticrack CRC runs when these overlays are loaded anyway). As such, the tool doesn't need to touch those.
